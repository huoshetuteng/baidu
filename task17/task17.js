...
function renderChart() {
    ...
    for(var item in chartData){
        ...
        text += '<div title="'+chartData[item]+'" style="height:'+chartData[item]+'px;background:'+color+'"></div>';
    }
...
function initAqiChartData() {
  ...
  // 处理好的数据存到 chartData 中
    ...
    if (pageState.nowGraTime=="month") {
        chartData = {};
        var daySum2=0,countSum2=0,month=0;
        for(var item2 in nowCityData){
            countSum2 += nowCityData[item2];
            daySum2++;
            if(new Date(item2).getMonth()!==new Date(item2).setDate((new Date(item2).getDay()+1)))
                month++;
                chartData['第'+month+'月'] = Math.floor(countSum2/daySum2);
                countSum2=0;
                daySum2=0;
        }
        if (daySum2!==0) {
